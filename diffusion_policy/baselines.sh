...
# =============================================================================
# state+rgb multi-modal baselines (NEW)
# Uses both proprioceptive state and RGB images for richer observation
# Requires trajectory files in state_dict+rgb format
# To generate: python -m mani_skill.trajectory.replay_trajectory \
#   --traj-path <original_traj.h5> -o state_dict+rgb -c <control_mode> --save-traj
# =============================================================================
for demos in 100; do
  ...
  python train_state_rgb.py --env-id PushCube-v1 \
    --demo-path ~/.maniskill/demos/PushCube-v1/motionplanning/trajectory.state_dict+rgb.pd_ee_delta_pos.physx_cpu.h5 \
    --control-mode "pd_ee_delta_pos" --sim-backend "physx_cpu" --num-demos ${demos} --max_episode_steps 100 \
    --total_iters 30000 \
    --exp-name diffusion_policy-PushCube-v1-state_rgb-${demos}_motionplanning_demos-${seed} \
    --demo_type=motionplanning --track

  python train_state_rgb.py --env-id StackCube-v1 \
    --demo-path ~/.maniskill/demos/StackCube-v1/motionplanning/trajectory.state_dict+rgb.pd_ee_delta_pos.physx_cpu.h5 \
    --control-mode "pd_ee_delta_pos" --sim-backend "physx_cpu" --num-demos ${demos} --max_episode_steps 200 \
    --total_iters 100000 \
    --exp-name diffusion_policy-StackCube-v1-state_rgb-${demos}_motionplanning_demos-${seed} \
    --demo_type=motionplanning --track

  python train_state_rgb.py --env-id PegInsertionSide-v1 \
    --demo-path ~/.maniskill/demos/PegInsertionSide-v1/motionplanning/trajectory.state_dict+rgb.pd_ee_delta_pose.physx_cpu.h5 \
    --control-mode "pd_ee_delta_pose" --sim-backend "physx_cpu" --num-demos ${demos} --max_episode_steps 300 \
    --total_iters 100000 \
    --exp-name diffusion_policy-PegInsertionSide-v1-state_rgb-${demos}_motionplanning_demos-${seed} \
    --demo_type=motionplanning --track
done